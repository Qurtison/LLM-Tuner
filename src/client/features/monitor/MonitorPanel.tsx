...
function MiniChart({ metric, points, smooth, master, worker, net, masterLabel, workerLabel }: { metric: MiniMetric; points: Point[]; smooth: boolean; master: Stats | null; worker: Stats | null; net: number | null; masterLabel: string; workerLabel: string }) {
...
export default function MonitorPanel() {
    ...
    return <section className="@container space-y-4" aria-label="Telemetry monitor">
        ...
        <div className="grid gap-2 grid-cols-2 @lg:grid-cols-4">{miniMetrics.map(metric => <MiniChart key={metric.key} metric={metric} points={points} smooth={smooth} master={master} worker={worker} net={net} masterLabel={masterLabel} workerLabel={workerLabel} />)}</div>
