...
const METRIC_ORDER_KEY = 'monitor_metric_order';
...
function MiniChart({ metric, points, smooth, master, worker, net, masterLabel, workerLabel, tpsPoints }: { metric: MiniMetric; points: Point[]; smooth: boolean; master: Stats | null; worker: Stats | null; net: number | null; masterLabel: string; workerLabel: string; tpsPoints: TpsPoint[] }) {
...
export default function MonitorPanel() {
    ...
    // User-reorderable row order (drag the grip), persisted; the lock freezes it.
    const [order, setOrder] = useState<string[]>(() => {
        const defaults = miniMetrics.map(metric => metric.key);
        try {
            const raw: unknown = JSON.parse(window.localStorage.getItem(METRIC_ORDER_KEY) || '[]');
            if (!Array.isArray(raw)) return defaults;
            const saved = raw.filter((key): key is string => typeof key === 'string' && defaults.includes(key));
    ...
    useEffect(() => { try { window.localStorage.setItem(METRIC_ORDER_KEY, JSON.stringify(order)); } catch { /* full or unavailable */ } }, [order]);
    useEffect(() => { try { window.localStorage.setItem(METRIC_LOCK_KEY, metricLocked ? '1' : ''); } catch { /* full or unavailable */ } }, [metricLocked]);
    const moveMetric = (from: string | null, to: string) => {
    ...
    return <section className="@container space-y-4" aria-label="Telemetry monitor">
        ...
        <div className="grid gap-2 grid-cols-1">
            {order.map(key => {
                ...
                return <div key={key} onDragOver={event => { if (!metricLocked) event.preventDefault(); }} onDrop={event => { event.preventDefault(); moveMetric(dragKey.current, key); }} className="flex items-stretch gap-1">
                    <span draggable={!metricLocked} onDragStart={event => { if (metricLocked) return; dragKey.current = key; event.dataTransfer.effectAllowed = 'move'; event.dataTransfer.setData('text/plain', key); }} onDragEnd={() => { dragKey.current = null; }} title={metricLocked ? undefined : 'Drag to reorder'} className={'flex select-none items-center px-1 text-sm ' + (metricLocked ? 'text-neutral-800' : 'cursor-grab text-neutral-500 hover:text-neutral-300 active:cursor-grabbing')} aria-hidden>&#10066;&#10066;</span>
                    <div className="min-w-0 flex-1"><MiniChart metric={metric} points={points} smooth={smooth} master={master} worker={worker} net={net} masterLabel={masterLabel} workerLabel={workerLabel} tpsPoints={tpsPoints} /></div>
                </div>;
            })}
        </div>
